...
const GITHUB_TOKEN = process.env.GITHUB_TOKEN; // Odczytujemy token z zmiennej środowiskowej

app.get("/", async (req, res) => {
  try {
    const response = await axios.get(GITHUB_API_URL, {
      headers: {
        Authorization: `token ${GITHUB_TOKEN}`, // Używamy tokena z zmiennej środowiskowej
      },
    });
    ...
    res.send(`
      <svg width="500" height="180" xmlns="http://www.w3.org/2000/svg">
        <style>
          ...
          .project { fill: #58a6ff; font-family: monospace; font-size: 16px; text-decoration: underline; }
